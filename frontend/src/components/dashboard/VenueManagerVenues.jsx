...
export default function VenueManagerVenues() {
  ...
  const fetchVenues = async () => {
    try {
      // First try to get all venues managed by this manager
      const response = await api.get('/venues');
      console.log('API Response:', response.data);
      
      // Filter venues that belong to this manager
      // (This assumes the API returns the manager info with venues)
      const managerVenues = response.data.filter(venue => 
        venue.manager && venue.manager.id === user.id
      );
      
      // If we can't determine manager from the venue data, use all venues
      setVenues(managerVenues.length > 0 ? managerVenues : response.data);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Booked Venues</h1>
